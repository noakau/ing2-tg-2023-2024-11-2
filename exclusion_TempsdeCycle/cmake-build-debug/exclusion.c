...
void assignOperationsToStations(int* operations, int num_operations, Exclusion* exclusions, int numExclusions) {
    ...
    for (int i = 0; i < num_operations; i++) {
        ...
        for (int j = 0; j < num_stations; j++) {
            ...
            if (canBeAssigned) { // si on peut assigner deux opérations à la même station
        ...
        if (!assigned) { // si l'opération n'est pas déjà assignée à une station
            // Créer une nouvelle station et assigner l'opération
            num_stations++;
            stations = (Station*)realloc(stations, num_stations * sizeof(Station));
            stations[num_stations - 1].operations = (int*)malloc(sizeof(int));
            stations[num_stations - 1].operations[0] = operations[i];
            stations[num_stations - 1].nb_operations = 1;
        }
    }
