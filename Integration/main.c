...
#include "ExclusionAndCycleTime.h"
    ...
    do {
        ...
        if(choix ==2){
            ...
            for (int i = 0; i < total_operations; i++) {
                printf("Operation : %d, Temps d'execution : %.2f, Station : %d\n", operations[i].operation, operations[i].execution_time, operations[i].assigned_station);
            }
        ...
        if(choix==4){
            int choix2;
            printf("quelles contraintes voulez-vous lancer en même temps\n");
            printf("1. Test de la contrainte d'exclusion et de temps de cycle\n");
            printf("2. Test de la contrainte d'exclusion et de precedence\n");
            printf("Choix : ");
            scanf("%d", &choix2);
            if(choix2==1){
                printf("Veuillez entrer le nom du fichier d'exclusions : \n ");
                ...
                // Calcul du temps de cycle d'une operation
                double StationTime= calculateStationCycleTime( Workstation[0], &operations, total_operations);
                printf("temps de stations 1: %.8f", StationTime);
                // pause
                gets(Exclusionfilename);

                // Calcul du temps de cycle global
                double TotalStationTime = CalculateMaxStationCycleTime( Workstation, nbStations, &operations, total_operations);
                printf("le max de temps est %2f",TotalStationTime );

                Workstation = AssignOpToStationsGivenExclusionAndTiming(operations, num_operations, exclusions, numExclusions, tempsCycle, nbStations);
                gets(operationfilename);
                for(int i = 0; i<nbStations; i++){
                    double station_cycle_time = calculateStationCycleTime(Workstation[i], operations, num_operations);
                    printf("\nTemps de cycle de la station %d : %.2f\n", i + 1, station_cycle_time);
                }
